...
// 1、自定义的校验方法
func nameNotNullAndAdmin(v *v8.Validate, topStruct reflect.Value, currentStructOrField reflect.Value, field reflect.Value, fieldType reflect.Type, fieldKind reflect.Kind, param string) bool {
...
func main2() {
	...
	if v, ok := binding.Validator.Engine().(*v8.Validate); ok {
...
func bookableDate(
	v *v8.Validate, topStruct reflect.Value,
	currentStruct reflect.Value, field reflect.Value, fieldtype reflect.Type,
	fieldKind reflect.Kind, param string,
) bool {
...
// 多语言翻译验证 - 同一个接口，支持多方向，多业务参数校验
/*
当业务系统对验证信息有特殊需求时，
例如：返回信息需要自定义，手机端返回的信息需要是中文
而pc端发挥返回的信息需要时英文，
如何做到请求一个接口满足上述三种情况。
*/
